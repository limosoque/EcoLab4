...
#define MAX_STATIC_TASK_COUNT   7
...
int16_t ECOCALLMETHOD CEcoTaskScheduler1Lab_C761620F_NewTask(/*in*/ IEcoTaskScheduler1Ptr_t me, /*in*/ voidptr_t address, /*in*/ voidptr_t data, /*in*/ uint32_t stackSize, /* out */ IEcoTask1** ppITask) {
    ...
    for (indx = 0; indx < MAX_STATIC_TASK_COUNT; indx++) {
        if (g_xCEcoTask1List_C761620F[indx].pfunc == 0) {
			...
			g_xCEcoTask1List_C761620F[indx].runtime = (uint32_t*)&data[0];
 ...
 *
 */
void InsertionSort(/*in*/ CEcoTaskScheduler1Lab_C761620F* pCMe, /* in */ uint64_t* arr, /* in */ uint64_t arr_size) {
	uint64_t i, j, temp;

	for (i = 0; i < arr_size - 1; ++i) {
		for (j = i + 1; j < arr_size; ++j) {
			if (pCMe->m_pTaskList[arr[i]].runtime < pCMe->m_pTaskList[arr[j]].runtime) {
				temp = arr[i];
				arr[i] = arr[j];
				arr[j] = temp;
			}
...
int16_t ECOCALLMETHOD CEcoTaskScheduler1Lab_C761620F_Start(/*in*/ IEcoTaskScheduler1Ptr_t me) {
    CEcoTaskScheduler1Lab_C761620F* pCMe = (CEcoTaskScheduler1Lab_C761620F*)me;
	uint64_t tasks[MAX_STATIC_TASK_COUNT], i, tasks_size;
	...
	for (i = 0; i < MAX_STATIC_TASK_COUNT && pCMe->m_pTaskList[i].pfunc != 0; ++i) {
		tasks[i] = i;
	}
    tasks_size = i;

    InsertionSort(pCMe, tasks, tasks_size);

	for (i = 0; i < tasks_size; ++i) {
		pCMe->m_pTaskList[tasks[i]].pfunc();
	}
